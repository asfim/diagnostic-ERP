...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST">
            ...
            @php
                // Group permissions by the second word
                $groupedPermissions = [];
                    ...
                    <div class="card h-100 shadow-sm border-0" style="border-radius: 10px; overflow: hidden;">
                        ...
                        <div class="card-body bg-light module-body-{{ str_replace(' ', '_', $module) }}">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
                                ...
                                <div class="form-check form-switch m-0">
                                    <input class="form-check-input permission-checkbox" type="checkbox" name="permissions[]" value="{{ $permission->name }}" id="perm_{{ $permission->id }}" {{ in_array($permission->name, $rolePermissions) ? 'checked' : '' }}>
                                </div>
                            </div>
                            @endforeach
                        </div>
            ...
            <div class="mt-4 text-end">
                <button type="submit" class="btn btn-primary px-4 rounded-pill shadow"><i class="fa-solid fa-save me-1"></i> Update Role</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // Function to initialize the select all toggles based on child checkboxes
    document.addEventListener("DOMContentLoaded", function() {
        const modules = document.querySelectorAll('.card-header');
        modules.forEach(header => {
            const selectAll = header.querySelector('.module-select-all');
            if (selectAll) {
                const moduleName = selectAll.getAttribute('onclick').match(/'([^']+)'/)[1];
                const checkboxes = document.querySelectorAll('.module-body-' + moduleName + ' .permission-checkbox');
